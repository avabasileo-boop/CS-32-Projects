...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    {
        Store currentStore(s[i]);
        size_t j = i;
        for (; j > 0 && comp(currentStore, s[j - 1]); j--)
            s[j] = s[j - 1];
        s[j] = currentStore;
    }
...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    vector<Store*> storesPtrs;
    for (size_t k = 0; k < auxStores.size(); k++)
    {
        storesPtrs.push_back(&auxStores[k]);
    }
    ...
    sort(storesPtrs.begin(), storesPtrs.end(), comp);
    ...
    for (size_t k = 0; k < storesPtrs.size(); k++)
    {
        stores[k] = *storesPtrs[k];
    }
