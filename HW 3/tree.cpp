//QUESTION #4
// For this part of the homework, you will turn in one file named tree.cpp that 
//contains the four functions above and nothing more.
...
{
    //empty sequence appears in a1 1 time, even if a1 is empty
    //OR: return 1 when you get a2, meaning a2 was found in a1 
    if (n2 <= 0)
    {
        return 1;
    }

    //got past a1 and still searching
    //meaning a2 was not found in a1 
    if (n1 <= 0)
    {
        return 0;
    }

    //element in a1 matches element in a2
    if (a1[0] == a2[0])
    {
        //Compare next element in a2 against next element in a1
        //PLUS result of check if a2 array is in a1 array going forward 
        return (countIncludes(a1 + 1, n1 - 1, a2 + 1, n2 - 1) + countIncludes(a1 + 1, n1 - 1, a2, n2));
    }
    //element in a1 does NOT match element in a2 
    else
    {
        //compare current a2 element against next a1 element 
...
{
    //end ordering once n is one element long 
    //return if a is 1 or fewer elements long 
    if (n <= 1)
    {
        return; 
    }
    ...
    int firstNotLess; 
    int firstGreater; 
    separate(a, n, *a, firstNotLess, firstGreater); 
    order(a, firstNotLess);                    //order array 1
    order(a + firstGreater, n - firstGreater); //order array 2
}
